...
__global__ void softmax(int  w, int h, float* input, float* output){
    ...
    if (row < h && column < w){
        ...
        output[row*w + column] = exp(input[row*w + column] - maxval) / (divisor);
    }
}
...
__global__ void cross_entropy_backwards(int w, int h,
    ...
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    if (row < h && column < w){
        output[row * w + column] = preds[row * w + column] - real[row * w + column];
     }
}
...
void ensure_dir(const char* d) {
    ...
    _mkdir(d);
#else
    mkdir(d, 0755);   // ok if it already exists
#endif
}

void save_bin(const char* path, const float* dptr, size_t count) {
    std::vector<float> host(count);
    gpuErrchk(cudaMemcpy(host.data(), dptr, count*sizeof(float), cudaMemcpyDeviceToHost));
    FILE* f = std::fopen(path, "wb");
    ASSERT(f, "failed to open %s", path);
    std::fwrite(host.data(), sizeof(float), count, f);
    std::fclose(f);
}
...
int main(int argc, char** argv){
    ...
    save_bin("artifacts/weights1.bin", weights1, (size_t)784 * size1);
    save_bin("artifacts/biases1.bin",  biases1, (size_t)size1);
    save_bin("artifacts/weights2.bin", weights2, (size_t)size1 * size2);
    save_bin("artifacts/biases2.bin",  biases2, (size_t)size2);
    save_bin("artifacts/weights3.bin", weights3, (size_t)size2 * 10);
    save_bin("artifacts/biases3.bin",  biases3, (size_t)10);
    std::cout << "Saved weights to ./artifacts/*.bin" << std::endl;
}